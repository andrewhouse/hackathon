json.array!(@companies) do |company|
  ...
  json.employees company.employees do |employee|
    ...
    json.folders employee.folders do |folder|
      json.folder folder, :title, :description, :employee_id, :created_at, :updated, :size
      json.items folder.items do |item|
        json.item item, :title, :created_at, :record_file_name, :record_content_type, :record_file_size, :record_updated_at, :s3_url
      end
    end
  end
end
